...
public class CCFile {
    ...
    internal var mDataAreaSize : Int = 0;
    ...
    public func parseCCFile(buffer : [UInt8]) throws {
        ...
        if (buffer[2] == 0x00) {
            ...
            mDataAreaSize = Int(CCFile.CCFILE_DATA_AREA_SIZE_MULTIPLIER * Int((buffer[6] << 8) + buffer[7]))
        } else {
            if(buffer.count != 4) {
                //throwException() /* throw STException(INVALID_CCFILE); */
            }
            ...
            mCCLength = buffer.count;
            mDataAreaSize = Int(CCFile.CCFILE_DATA_AREA_SIZE_MULTIPLIER * Int(ComStSt25sdkHelper.convertByteToUnsignedInt(withByte: jbyte(buffer[2]))))
