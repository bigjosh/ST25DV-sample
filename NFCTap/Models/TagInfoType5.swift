...
class TagInfoType5: NSObject {
    ...
    var mProductId = ProductID.PRODUCT_UNKNOWN
    ...
    private func processTagForProductSTMUntracebale() {
        var pdt : ProductID = ProductID.PRODUCT_ST_STM_UNTRACEABLE
        if self.mTagSystemInfo != nil {
           self.mTagSystemInfo!.setProductID(productID: pdt)
        }
        self.mProductId = pdt
    ...
    private func buildUiCCFileStructureData() {
        ...
        self.mTagPropertiesInformationTableModel.buildCCFileInfo(miOSCCFileType5:self.miOSCCFileType5!)
    }
